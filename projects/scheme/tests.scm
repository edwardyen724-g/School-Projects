...
(quote (cdr x))
; expect (cdr x)
(eval (define x (quote (cdr x))))
; expect (cdr x)
(quote (1 .(2)))
; expect (1 2)
(begin (define x (cons 1 . (2))) x)
; expect (1 . 2)
(begin (quote (1 2))(define x '(1 2)))
; expect x
(lambda (y) (+ 1 y))
; expect (lambda (y) (+ 1 y))
(lambda (x y) (* x y))
...
(cond (False (/ 1 0))
      ...
      (else 'hey))
; expect true
(cond ((> 3 4) 'cannot math)
      (else '(still cannot pass math 1b)))
; expect (still cannot pass math 1b)
(define mydreamscore 200)
(cond ((> mydreamscore 296) 'a+)
      ((> mydreamscore 259) 'b+)
      (else '(cannot even declare)))
; expect (cannot even declare)
(define x 200)
; expect x
(define y 300)
; expect y
(let ((x 2)(y (+ 5 x))) (cons x y))
; expect (2 . 205)
(let ((a 1))
        (let ((x a)
             (y 3))
