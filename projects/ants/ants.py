...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(self.ant, QueenAnt) and not insect.imposter:
                return
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if isinstance(self.ant.ant, QueenAnt) and not insect.imposter:
                        return
                    else:
                        self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        range_count = 0
        while current_place != hive and current_place:
            bees = current_place.bees
            if bees and self.min_range <= range_count <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        def been_doubled(key):
            if key in self.doubling_list:
                if self.doubling_list[key]:
                    return True
            else:
                self.doubling_list[key] = True
                return False
        place = self.place
        if self.imposter:
            ...
            while place.exit:
                place = place.exit
                if place.ant:
                    if not been_doubled(place.ant):
                        place.ant.damage *= 2
                        if place.ant.container:
                            if place.ant.ant and not been_doubled(place.ant.ant) :
